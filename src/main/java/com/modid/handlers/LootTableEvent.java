...
public class LootTableEvent {
	...
	public static void customLootTableDrop(LivingDropsEvent event) {
		...
		if(event.getEntityLiving() instanceof EntityCow) {
			// 25% chance to drop the item
			if(rand.nextInt(4) == 0) {
				event.getEntityLiving().entityDropItem(new ItemStack(ItemInit.AMBER, 1), 0.0f);
			}
		}
	}
	...
	public static void customLootTableDrop(LootTableLoadEvent event) {
		System.out.println("[ModName] Loot table loaded: " + event.getName());

		String eventName = event.getName().toString();

		if(eventName.equals("minecraft:chests/desert_pyramid")) {
			event.getTable().addPool(amberPool());


		} else if(eventName.equals("minecraft:chests/simple_dungeon")) {
			event.getTable().addPool(exampleEnchantedSword());
		}
	}
	...
	private static LootPool exampleEnchantedSword() {
		...
		LootEntryItem swordEntry = new LootEntryItem(
			ItemInit.EXAMPLE_SWORD,  // ItemStack representing the item to be dropped
